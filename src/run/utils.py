...
def translate(model, src, vocab):
    model.eval()

    input_method = "soft-alignment"
    if input_method == "connection":
        num_videos = 2
        src = [src[:, i] for i in range(num_videos)]
        src = torch.cat(src, dim=1)
        num_tokens = src.shape[1]       # src.shape = (batch, seq_len, feature_dim)
    else:
        num_tokens = src.shape[2]       # src.shape = (batch, num_videos, seq_len, feature_dim)
    src_mask = (torch.zeros(num_tokens, num_tokens)).type(torch.bool)
    tgt_tokens = greedy_decode(
        model, src, src_mask, max_len=num_tokens + 5, start_symbol=BOS_IDX).flatten()
    return " ".join(vocab.lookup_tokens(list(tgt_tokens.cpu().numpy()))).replace("<bos>", "").replace("<eos>", "").strip()
    ...
    with tqdm(train_dataloader) as pbar:
        ...
        for src, tgt in pbar:
            ...
            if input_method == "connection":
                ...
                src = torch.cat(src, dim=1)
                # print(src.shape)
                src_mask, tgt_mask, src_padding_mask, tgt_padding_mask = create_mask_for_connection(src, tgt_input)
            else:
